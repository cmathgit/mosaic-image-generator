...
BLEND_ALPHA = 0.3 # Controls the strength of the color blend (0.0 = original, 1.0 = full color)
...
# --- Expanded 12-Color Versatile Palette ---
COLOR_PALETTE = {
    # Primaries & Secondaries (6)
    "WarmRed_Vermilion": hex_to_rgb("#E34234"),
    "CoolRed_QuinacridoneMagenta": hex_to_rgb("#8E3A59"),
    "WarmYellow_CadmiumYellowDeep": hex_to_rgb("#F6A623"),
    "CoolYellow_LemonYellow": hex_to_rgb("#FDFD96"),
    "WarmBlue_Ultramarine": hex_to_rgb("#3F51B5"),
    "CoolBlue_Cerulean": hex_to_rgb("#00A6D6"),
    # Earth Tones (3)
    "EarthyBrown_BurntUmber": hex_to_rgb("#8A4B28"),
    "EarthyYellow_YellowOchre": hex_to_rgb("#C5A059"),
    "EarthyRed_TerraCotta": hex_to_rgb("#E2725B"),
    # Neutrals + Modifiers (2)
    "White_TitaniumWhite": hex_to_rgb("#FFFFFF"),
    "Black_IvoryBlack": hex_to_rgb("#2D2D2D"), # Alt: Payne's Gray (#404958)
    # Accent Color (1)
    "Accent_Teal": hex_to_rgb("#008080"), # Alt: Violet (#8A2BE2), Emerald (#50C878)
}

# --- Helper Functions ---

def adjust_image_color(image_path, output_path, color_name, target_rgb, blend_alpha):
    """
    Adjusts the color of an image by blending it with a solid color layer.
    ...
    Args:
        ...
        blend_alpha (float): The blending factor (0.0 to 1.0).
    """
    try:
        with Image.open(image_path) as img:
            # Ensure image is in RGB format for blending
            img_rgb = img.convert("RGB")

            # Create a solid color layer of the same size
            color_layer = Image.new("RGB", img_rgb.size, target_rgb)

            # Blend the original image with the color layer
            blended_img = Image.blend(img_rgb, color_layer, blend_alpha)

            # --- Optional Further Adjustments (Example: Enhance Contrast Slightly) ---
            # You could add subtle BCG (Brightness, Contrast, Gamma) or HSL adjustments here
            # if needed, operating on the 'blended_img'.
            # Example: Slightly increase contrast
            # enhancer = ImageEnhance.Contrast(blended_img)
            # final_img = enhancer.enhance(1.1) # Increase contrast by 10%
            # blended_img.save(output_path) # Save the blended image directly
            # Instead of final_img.save(output_path) if only blending is needed.

            blended_img.save(output_path)
            logging.info(f"Created '{color_name}' version (blended): {output_path}")
...
# --- Pre-execution Helper Functions --- START ---
...
# --- Pre-execution Helper Functions --- END ---
...
def main():
    ...
    for filename in input_filenames:
        ...
        # 3b. Create Blended Versions
        for color_name, target_rgb in COLOR_PALETTE.items():
            # Ensure color names are filesystem-friendly
            safe_color_name = color_name.replace(" / ", "_").replace(" ", "")
            output_filename = f"{base_name}_{safe_color_name}{ext}"
            output_image_path = os.path.join(OUTPUT_DIR, output_filename)
            # Pass BLEND_ALPHA to the function
            # Assuming adjust_image_color logs its own errors
            adjust_image_color(input_image_path, output_image_path, color_name, target_rgb, BLEND_ALPHA)
